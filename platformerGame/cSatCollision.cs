...
namespace platformerGame
{
    ...
    class cSatCollision
    {
        ...
        const float c_fFriction = 0.1f; //0.02f;
        const float c_fRestitution = 0.5f; //0.1f;
        const float c_fGlue = 0.001f; //0.01f;
        ...
        public static bool checkAndResolve(cGameObject objA, cGameObject objB, float dt, bool resolve = false)
        {
           ...
           // Vector2f RelPos = objA.Position - objB.Position;
            Vector2f RelDis = objA.Velocity - objB.Velocity;

            if(Collide(objA.Bounds.getLocalVertices(), 4,
                                  objB.Bounds.getLocalVertices(), 4,
                                  RelPos, RelDis,
                                  ref N, ref t))
            {
                if(resolve)
        ...
        private static void ProcessCollision(cGameObject objA, cGameObject objB, Vector2f N, float t)
        {
            ...
            float CoF = c_fFriction;

            if (dt < c_fGlue * c_fGlue) CoF = 1.01f;

            //D = -(1.0f + s_fRestitution) * Dn - (CoF) * Dt;

            D.X = -(1.0f + c_fRestitution) * Dn.X - (CoF) * Dt.X;
            D.Y = -(1.0f + c_fRestitution) * Dn.Y - (CoF) * Dt.Y;
        ...
        private static void ProcessOverlap(cGameObject objA, cGameObject objB, Vector2f xMTD)
        {
            ...
            {
                objA.MoveBy(mtd * 0.5f);
                objB.MoveBy(mtd * -0.5f);
                                     ...
                                     ref float t )
        {
            ...
            if (!FindMTD(xAxis, taxis, iNumAxes, N, ref t))
                return false;

            // make sure the polygons gets pushed away from each other.
            if (cAppMath.Vec2Dot(N, xOffset) < 0.0)
                N = -N;
        ...
        private static bool FindMTD(Vector2f[] xAxis, float[] taxis, int iNumAxes, Vector2f N, ref float t)
        {
